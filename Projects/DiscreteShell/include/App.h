...
public:
    ...
    void sceneCallback()
    {
        ...
        if (!animate_modes && run_sim)
        {
            ...
            if(simulation.set_window) psMesh->addFaceScalarQuantity("homogenised strain magnitude", simulation.homo_strain_magnitudes);
            ...
            auto nodal_force = simulation.external_force;
            simulation.computeTestWindowForces(nodal_force);
            Eigen::MatrixXd nod_force;
            vectorToIGLMatrix<T, 3>(nodal_force, nod_force);
            psMesh->addVertexVectorQuantity("elastic forces", nod_force);
    ...
    std::vector<Eigen::Vector3d> updateLinePosition(Simulation& sim){
        ...
        for(size_t i = 0; i < sim.sample.size(); ++i){
            ...
            // auto F_2D_inv = sim.optimization_homo_target_tensors[tri].lu().solve(Matrix<T, 2, 2>::Identity());
            Matrix<T, 3, 3> F_inv = Matrix<T, 3, 3>::Zero();
            auto F = F_inv;
            F.block(0,0,2,2) = sim.optimization_homo_target_tensors[tri];
